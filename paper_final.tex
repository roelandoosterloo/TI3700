...
\title{Seminarium TI 3700\\ A survey on Twitter Data Analysis}
...
were reviewed, being the linear-weighted hybrid model, and the pairwise interaction tensor factorization. We discussed the 
concept of event identification which can be separated in two problems (1) Separating event and non-event content and (2) Aligning Tweets with an Event. We presented solutions for these problems and we have shown that event identification can be a useful technique in practice. (e.g. It can be used to detect Earthquakes)

It got clear that there is always room for improvement in the existing techniques. For example, as mentioned earlier, in sentiment classification, there is room for improvement since its performance falls behind those of subject classification. We suspect this might be a matter of figuring out what features to use. We expect an increased interested from companies for the results of their advertisement campaigns. This would pose an interesting field for future research. 
