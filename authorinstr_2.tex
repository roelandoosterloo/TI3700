...
Microblogging has grown out to be one of the most interesting forms of social media. One of its most popular implementations has to be Twitter, where
users can share short messages, called Tweets, with every other user. The limitation in characters forces users to 
compress all their information into a small message, making them extra interesting to analyse. These Tweets often contain information broadcasted by the user in
the form of opinions, emotions, interests, etc. One of the biggest challenges is finding a way to extract the intended information from these Tweets. Once
the information is extracted however, the possible applications are endless. This paper is dedicated to combining succesful techniques and useful applications
of these social web data analytics. In the following section, Twitters API gets introduced which will show that the platform already offers great tools
and commands to retrieve specific datasets, or a completely random sample stream of Tweets. After that, two entire sections will be dedicated to all sorts
of techniques that are being used to extract this information. One section will focus on single tweets, and the following on sets of multiple Tweets. Both
of these sections will involve several usecases for certain techniques.
...
used by researchers to model the information of tagged based analysis, The Linear-Weighted Hybrid Model, and the Pair-wise Interaction Tensor Factorisation Model.
...
All weights $\alpha_i$ are then computed by applying a random-restart hill climbing technique. A subset is used as a holdout set, and a vector
gets created with random weights that sum to 1. The recommender then modifies the vector over the holdout set to train the algorithm until it reaches
a stable point. The algorithm then restarts to avoid local minima.

\item \textbf{Pair-Wise Interaction Tensor Factorization} 

The Pair-Wise Interaction Tensor Factorisation is a state-of-the-art algorithm that has
...
The adaptation involves changing the roles of resources and tags with respect to each other.
	
	
	
	

Our adaptation of the model to resource
...
source selection). An iterative gradient-descent algorithm is employed to optimize a116
J. Gemmell et al.
ranking function that prefers positive examples in the data over negative ones. Each of
four related matrices is updated until convergence is found. The matrices represent the
U
T
factor-reduced components of the specialized tensor factorization M = Uk Rk
+Tk Rk
,
U
where Uk is the user factor matrix, Tk is the tag factor matrix, Rk is the resource factor
T
matrix with respect to users and Rk
is the resource factor matrix with respect to tags, k
...
relevance score of a resource given a user-tag pair is calculated as: φ(u, {t}, r) =
